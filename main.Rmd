...
decision_tree_model <- rpart(
  ...
  control = rpart.control(cp = 0.01) # Complexity parameter for pruning
)

# Visualize the decision tree
rpart.plot(decision_tree_model, type = 2, extra = 102, under = TRUE)

# Predict on the test set
dt_predictions <- predict(decision_tree_model, newdata = test, type = "class")

# Evaluate predictions with a confusion matrix
confusion_matrix <- table(Predicted = dt_predictions, Actual = test$Result)
...
# Create a raw decision tree model
dt_model <- rpart(
  Result ~ .,          # Formula (target variable ~ predictors)
  data = train,        # Training dataset
...
# Visualize the decision tree
rpart.plot(dt_model, main = "Decision Tree for NBA Results")
