...
```{r}
#changing rank names and binding all files
...
```{r}
#removing wins that don't make it to playoffs
...
```{r}
# adding year as suffix to abbreviation in rank dataset
combined_data <- combined_data |>
...
```{r}
# adding year as suffix to abbreviation in win dataset
combined_wins <- combined_wins |>
  transform(Name_Year = paste(Name, Year, sep = "_")) |>
  select(-Name, -Year)
```
...
```{r}
# reading in WAR file, adding year as suffix to team name, and averaging WAR by Name_Year to give overall team WAR 
...
```{r}
# merging all three datasets on Name_Year
